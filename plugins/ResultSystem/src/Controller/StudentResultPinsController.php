...
class StudentResultPinsController extends AppController
{
    ...
    public function excelFormat()
    {
        if ( $this->request->is(['post']) ) {
            ...
            $applicationPins = $this->StudentResultPins->find('all')->select(['serial_number','pin'])->where(['student_id IS NULL',['DATE(created)'=>(new Time($this->request->data['created']))->i18nFormat('yyyy-MM-dd')]])->hydrate(false)/*->toArray()*/;
